...
async function readFromReplitDB(): Promise<LocalConfig> {
...
async function writeToReplitDB(config: LocalConfig): Promise<void> {
  if (!REPLIT_DB_URL) return;
  try {
    ...
    console.warn("[local-config] Erro ao salvar no Replit DB:", e);
  }
}
...
export async function readLocalConfigAsync(): Promise<LocalConfig> {
  if (REPLIT_DB_URL) {
    // Em produção: lê do Replit DB (persiste para sempre)
    const dbCfg = await readFromReplitDB();
...
export function writeLocalConfig(config: LocalConfig): void {
  ...
  writeToFile(config);
  // Salva no Replit DB em background (persiste em produção)
  if (REPLIT_DB_URL) {
    writeToReplitDB(config).catch(() => {});
  }
}
...
export async function applyLocalConfigToEnv(): Promise<void> {
  // Carrega do Replit DB se disponível (produção), senão do arquivo
  ...
  if (REPLIT_DB_URL) {
    console.log("[local-config] Configurações carregadas do banco permanente Replit");
  }
}
